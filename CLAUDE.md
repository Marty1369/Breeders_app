...
Still open / known gaps: contract PDF generation can't render Lithuanian chars (parked); per-breed colour catalogs, user-creatable expense/health categories, a health-log calendar view, and a role/buyer-management pass are on the improvements plan; notifications are in-app only (no web push). `docs/ENVIRONMENTS.md` governs deploys (staging-first: apply any new migration to the prod DB **before** the code that reads it merges to `main`). An uncommitted PDF-Unicode font-embed (`@pdf-lib/fontkit` + NotoSans TTFs) sits in the working tree but is not yet committed/shipped.
