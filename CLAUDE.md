...
Feature-complete against the prototype. **A large overhaul is in progress on the `staging` branch** (Vercel preview `breeders-app-git-staging-marty-inc.vercel.app`, staging DB) and NOT yet on prod. Prod (`main`) has only the two security fixes (0007, 0009) applied.

Landed on staging since the baseline:
